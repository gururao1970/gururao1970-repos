...
<xsl:variable name = "ServletBase">"http://localhost:8080/scamazon/servlet/Catalog"></xsl:variable>
...
<div style='float: right;'><a href="http://localhost:8080/scamazon/servlet/Cart?Show=show">Show  Cart</a></div>
...
<xsl:variable name="MyDescriptor">
 ...
 <xsl:value-of select="price"/>
  ...
  <xsl:text>http://localhost:8080/scamazon/servlet/Catalog?Buy=</xsl:text>
  <xsl:value-of select="$TypeKey"/>
  
 <xsl:text>&amp;itemDes=</xsl:text>
<xsl:value-of select="$itemDescription" />
  
  <xsl:text>&amp;LabelDes=</xsl:text>
   <xsl:value-of select="$Mytypes" />
